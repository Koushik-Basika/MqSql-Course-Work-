...
select  singer_name,followers from singers where followers>any
(select followers from singers where language="tamil");

--- all subqueries ---

select title,plays from songs where plays>all(select plays from songs where language="tamil");

select singer_name,followers from singers where followers>all
(select followers from singers where language="telugu");
...
select s1.title,s1.singer_id,s1.plays from songs s1 where s1.plays=
(select max(s2.plays) from songs s2 where s2.singer_id=s1.singer_id);

select s1.title,s1.likes from songs s1 where s1.likes>
(select avg(s2.likes) from songs s2 where s2.singer_id=s1.singer_id);
